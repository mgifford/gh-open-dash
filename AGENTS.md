...
3. **Rate-limit safe**
   ...
   - Use org-wide weekly queries and aggregate locally.
...
### GraphQL usage
- Use `search` with `type: ISSUE` and qualifiers:
  - `org:civicactions is:pr created:START..END`
  - `org:civicactions is:pr merged:START..END`
  - `org:civicactions is:issue created:START..END`
- Fetch only:
  - `author.login`
...
- Default initial history window is 52 weeks.
